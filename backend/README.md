...
每次提交占一行。表格前两行是分组表头：每个指标下面固定有 `ours`、`c2w`、`viga`、`direct`、`mcp`、`swe` 六列，便于直接横向比较；提交时间、昵称、组编号、完成方式和视频展示顺序位于左侧。`set_id` 是对应的 01–13 视频组编号，不是评分。

评分范围为 1–5 分整数。修改此后端文件后，需要在 Apps Script 中重新部署 Web App 新版本。

调查页会通过 `doGet?action=history` 查询同一昵称已经完成的组别，并从未完成的组别中继续随机选择。若已有旧版三方法或五方法数据，新的六方法数据会自动写入 `Responses_6_methods`，旧数据保留不变；历史查询会同时读取三个工作表。
